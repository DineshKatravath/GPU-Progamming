...
__global__ void CalculateInvertedGrayScale(long int* redMatrix, long int* greenMatrix,long int* blueMatrix,long int* T1Mat , int m ,int n){
  ...
  if(idx<(m*n)){
    unsigned x = idx/n;
    unsigned y = idx%n;
    unsigned invertedIdx = (m-1-x)*n + y;
    T1Mat[invertedIdx] =  floorf((redMatrix[idx]+greenMatrix[idx]+blueMatrix[idx])/3);
  }
}

__global__ void CalculateThomasTransformation(long int* redMatrix, long int* greenMatrix,long int* blueMatrix,long int* T2Mat , int m ,int n){

  unsigned idx = blockIdx.x*blockDim.x + threadIdx.x;

  if(idx<(m*n)){
    T2Mat[idx] = floorf(0.5*redMatrix[idx]) + sqrtf(greenMatrix[idx]) + blueMatrix[idx];
  }
